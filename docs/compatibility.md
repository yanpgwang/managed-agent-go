...
| Deployments | Limited | All ten Deployment and Deployment Run operations interoperate with the official Go SDK. A Deployment pins an Agent Version and binds an Environment, initial events, File or Memory Store resources, ordered Vaults, metadata, and an optional five-field cron schedule. Manual runs remain available while paused; archive is terminal; unpause resumes from the next future occurrence without backfilling. Scheduled execution uses PostgreSQL leases and idempotent occurrence keys, and requires the `orchestrate` worker role. GitHub repository resources, hosted scheduler jitter parity, and automatic Deployment archival when an Agent is archived are not implemented. |
...
| Multi-agent orchestration | Limited | Coordinator rosters accept all three official input forms, validate existence/lifecycle/depth/distinctness, and pin every entry to an immutable Agent Version. `self` follows the owning coordinator Version. The primary Session Thread is observable; child-thread creation, delegation, cross-thread messaging, cross-posted events, and orchestration are not yet executed. |
