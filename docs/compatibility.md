...
| Environment lifecycle | Supported | Create, get, list, update, archive, and delete support `cloud` and `self_hosted`; mutable resource fields, self-hosted scope, Environment type, default response shapes, and delete responses round-trip through the official SDK. Metadata update is a per-key patch, and archived Environments are read-only. List supports archive filtering and forward cursor pagination. Cloud sandbox tools execute on the worker; self-hosted sandbox tools park for `user.tool_result` from the client. Explicit unrestricted networking and empty package sets are accepted; limited networking and non-empty package installation remain incomplete and are rejected. |
...
| Event send and list | Limited | All core single-agent input event types are validated and durably processed: `user.message`, `user.interrupt`, `user.tool_confirmation`, `user.custom_tool_result`, self-hosted `user.tool_result`, `user.define_outcome`, and companion `system.message`. Their nested content, source, search-result, and rubric variants reject malformed or undocumented fields before admission. Server-emitted tool events use the documented variants: `agent.tool_use`/`agent.tool_result` for built-ins and `agent.mcp_tool_use`/`agent.mcp_tool_result` for MCP calls, the latter correlated by `mcp_tool_use_id`. Their use events report `evaluated_permission: allow` when execution proceeds and `ask` when the Session parks for confirmation. Targeted multi-agent interrupts remain unsupported. Model request and outcome spans carry correlated per-request usage. |
