...
| [Files](api/files.md) | Limited | Five operations with configured S3-compatible storage, crash-recoverable intents, downloadable Session Resource copies, and Docker publication of regular files beneath `/mnt/session/outputs` before idle. Client uploads are not downloadable; message content, outcome rubrics, non-Docker output publication, and distributed reconciliation remain open. |
