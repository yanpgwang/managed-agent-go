...
|---|---|---|
| Agent lifecycle | Supported | Create, get, list, version, update with optimistic concurrency, and archive. List Agents implements its five documented query parameters (`created_at[gte]`, `created_at[lte]`, `include_archived`, `limit` with the documented default 20 / maximum 100, and `page`) over the latest version of each agent, and returns the documented `data` + `next_page` envelope. `created_at[gt]`, `created_at[lt]`, and `order` are not part of this endpoint and are rejected rather than ignored. Result order (newest first) is a local choice: no `order` parameter is documented. |
...
| Environment lifecycle | Supported | Create, get, list, update, archive, and delete support `cloud` and `self_hosted`. Cloud sandbox tools execute on the worker; self-hosted sandbox tools park for `user.tool_result` from the client. List Environments implements its three documented query parameters (`include_archived`, `limit`, `page`) and returns `next_page`; it documents no `created_at` filters and no `order`, so those are rejected rather than ignored, and — because upstream documents no `limit` default or maximum here — the default 100 / maximum 1000 applied is a local choice rather than the List Agents 20/100. Update accepts the five documented body fields (`config`, `description`, `metadata`, `name`, `scope`), preserves omitted fields, requires `type` inside a supplied `config`, and deletes a metadata key on `null` or empty string (Session metadata deletes only on `null`). Two local choices: an in-place `config.type` change is rejected, and an archived environment is read-only. The effect of an environment update on already-running Sessions is not documented upstream; Mango does not propagate it — a Session keeps the environment it was admitted with. |
...
| Live message previews | Limited | Each `span.model_request_start` is durably appended before its provider request can emit an opt-in `agent.message` start/delta frame. Preview frames cross NATS and are never persisted; the authoritative message and correlated `span.model_request_end` follow when that model round completes. Thinking previews are not implemented. |
