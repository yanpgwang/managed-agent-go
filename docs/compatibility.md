...
| Session Threads | Limited | All five official Get/List/Archive and Thread Event list/stream operations interoperate with the official Go SDK. Every Session receives a durable primary thread in the same creation transaction; its agent, status, usage, timing, and event history are projections of the existing Session and ledger. Archiving the only supported primary thread archives the idle Session. Child threads, delegation, cross-posting, and targeted interrupts are not yet executed. |
