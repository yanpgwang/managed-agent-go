...
| Session lifecycle | Supported | Create from latest or pinned agent versions, apply session-local overrides, preserve an immutable resolved snapshot, get, list, update title, archive, and delete. Usage, timing stats, and outcome evaluations are live projections rather than placeholders. Unsupported resources and vaults are rejected at create time — a Mango limitation, since upstream accepts `vault_ids` there — so their required response arrays truthfully remain empty. Rejecting `vault_ids` on session update is *conformant*, not a gap: upstream documents that field as "not yet supported; requests setting this field are rejected." |
...
| SSE event stream | Supported | Streams new persisted events across API/worker processes. NATS wakes subscribers and PostgreSQL sequence reads repair missed notifications. Frames carry `event:` and `data:` only: no `id:` and no `retry:` lines are emitted, by design and matching upstream, which documents neither of those fields nor `Last-Event-ID`. The stream does not replay history; the documented reconnection procedure is application-level — open a new stream, seed seen event IDs from `GET /v1/sessions/{id}/events`, then tail live and skip duplicates. |
...
| Files, skills, memory, and vaults | Not supported | These product surfaces have no resource endpoints and no runtime behavior. File-backed outcome rubrics and file-sourced message content are rejected explicitly; inline or URL image/document content still participates in the provider transcript and context projection. Agent `skills` are *not* rejected: they are accepted, persisted, and echoed back unchanged, but are never mounted or executed. See Agent configuration. |
