...
resource and immutable Version slice separately from Agent/Session execution.
...
| Session lifecycle | Supported | Create from latest or pinned agent versions, apply session-local overrides, preserve an immutable resolved snapshot, get, list, archive, and delete. Update accepts `title`, a per-key `metadata` patch, and a session-local `agent.tools`/`agent.mcp_servers` full replacement that requires an idle session and applies from the next turn; update-time `vault_ids` is rejected, matching upstream. Usage, timing stats, outcome evaluations, and active File Resources are live projections. Create-time vault rejection remains a Mango limitation because upstream accepts it. |
| Session Resources | Limited | The five SDK operations and create-time File resources are implemented. Every attachment owns an independent downloadable File copy and a durable desired mount; Docker streams and atomically publishes it through a read-only bind mount, restores it after worker reattachment, and removes it after deletion. Update explicitly rejects the GitHub-only token field. Admission requires configured Files storage, a cloud Environment, and `MANAGED_AGENT_SANDBOX=docker`; local, self-hosted, and current remote providers return `422`. GitHub repository and Memory Store variants remain unsupported. |
...
| Skills | Limited | All nine custom Skill and immutable Skill Version resource operations interoperate with the official Go SDK when S3-compatible storage is configured. Upload validation enforces a single safe root, root `SKILL.md`, frontmatter metadata, directory/name matching, the 30 MB bound, and archive traversal/symlink rejection. PostgreSQL owns metadata and lifecycle intents; object storage owns canonical zip archives. Anthropic-managed Skills are not mirrored. Agent and Session Skill references are still stored opaquely: they are not validated, pinned, mounted, or exposed to the model yet, so this resource slice does not claim runtime execution. |
| Memory and vaults | Not supported | These product surfaces remain outside the core harness scope. Inline or URL image/document content still participates in the provider transcript and context projection. |
