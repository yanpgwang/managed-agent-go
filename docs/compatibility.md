...
| Multi-agent orchestration | Not supported | `multiagent` configuration can be stored and the primary Session Thread is observable, but roster resolution, child-thread creation, delegation, cross-thread messaging, and orchestration are not executed. |
