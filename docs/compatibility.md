...
| Session Threads | Limited | All five official Get/List/Archive and Thread Event list/stream operations interoperate with the official Go SDK. Every Session receives a durable primary Thread with an independent Agent, status, usage, and timing projection in the same creation transaction. The current single-Agent runtime synchronizes execution changes transactionally; primary events remain views over the existing Session ledger. Archiving the only runtime-produced primary Thread archives the idle Session. Child execution, delegation, cross-posting, and targeted interrupts are not yet implemented. |
