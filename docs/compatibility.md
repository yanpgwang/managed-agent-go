...
| Agent configuration | Limited | System, tools, MCP references, metadata, and model configuration execute. MCP server definitions accept only the documented `type`, `name`, and `url` fields; unsupported nested fields are rejected before an Agent version is stored. Skills and multi-agent orchestration are not yet executed. |
