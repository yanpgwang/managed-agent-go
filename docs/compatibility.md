...
| Environment lifecycle | Supported | Create, get, list, update, archive, and delete support `cloud` and `self_hosted`; mutable resource fields, self-hosted scope, Environment type, default response shapes, configured package lists, and delete responses round-trip through the official SDK. Metadata update is a per-key patch, cloud config updates preserve omitted networking/package fields, and archived Environments are read-only. List supports archive filtering and forward cursor pagination. Cloud sandbox tools execute after configured `apt`, `cargo`, `gem`, `go`, `npm`, or `pip` packages install on Docker or a remote isolated backend; a deployment using the host-process local backend rejects non-empty package configuration at admission. Self-hosted sandbox tools park for `user.tool_result` from the client. Limited networking remains incomplete and is rejected. |
