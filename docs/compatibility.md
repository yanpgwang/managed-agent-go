...
| Agent configuration | Limited | System, tools, MCP references, metadata, and model configuration execute. Nested `tools`, `mcp_servers`, `skills`, and `multiagent` payloads are shape-validated on create, update, and session overrides, so undocumented nested keys (including MCP credential-shaped fields the connector never accepts) are rejected with 400 instead of stored and echoed back. Skills and multi-agent orchestration are not yet executed, and multiagent rosters are not resolved against stored agents. |
