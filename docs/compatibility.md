...
| Environment lifecycle | Supported | Create, get, list, archive, and delete support `cloud` and `self_hosted`; their default response and delete shapes decode through the official SDK. List supports archive filtering and forward cursor pagination. Cloud sandbox tools execute on the worker; self-hosted sandbox tools park for `user.tool_result` from the client. Optional resource fields, configured networking/packages, and Environment update remain incomplete. |
