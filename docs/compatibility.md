...
| Session Threads | Limited | All five official Get/List/Archive and Thread Event list/stream operations interoperate with the official Go SDK. Coordinators receive private `list_agents`/`send_to_agent` tools that atomically create or resume persistent child Threads. Each child owns an immutable Agent snapshot, event ledger, provider transcript, model/tool loop, retry state, usage, stable Workflow identity, and isolated live previews while sharing Session infrastructure. Lifecycle/message cross-posts give the primary stream a condensed view, terminal child text is delivered as a report rather than duplicated as `agent.message`, reports wake a later coordinator synthesis turn, and Session status/usage aggregate across Threads. Cross-posted client-action requests and response routing, exact hosted preview suppression for report-only turns, global/targeted interrupts, child Workflow shutdown on archive/deletion, context snapshots, advisors, and shared list-cost budgets remain open. |
...
| Multi-agent orchestration | Limited | Coordinator rosters accept the ordinary ID string, Agent object, and `self` forms, validate existence/lifecycle/depth/distinctness/inference geography, and pin every ordinary entry to an immutable Agent Version. Session creation expands those pins into complete, immutable Agent snapshots under the same dependency-lock boundary; `self` follows the owning coordinator Version and inherits Session overrides. The durable runtime boundary can create multiple child copies from one callable roster member without re-reading Agent resources, rejects nested spawning, and isolates each Thread ledger. Coordinator tool wiring, child execution, cross-thread messaging, cross-posted lifecycle events, and targeted routing are not yet executed. The v1.62 `advisor` roster variant is intentionally rejected until its reserved identity, consultation lifecycle, redaction, and billing semantics are implemented independently from ordinary child Agents. |
