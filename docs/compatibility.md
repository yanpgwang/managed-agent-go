...
| SSE event stream | Supported | Streams new persisted events across API/worker processes. NATS wakes subscribers and PostgreSQL sequence reads repair missed notifications. It does not replay history or support `Last-Event-ID`. Frames carry only `event:`/`data:` lines plus periodic `:` comment keepalives that keep an idle stream alive through a proxy; no `id:` line and no `retry:` directive is emitted, by design, because an `id:` line would advertise `Last-Event-ID` resumption the contract does not define. |
