...
| Agent configuration | Limited | System, tools, MCP references, metadata, and model configuration execute. Admission rejects undocumented fields and malformed optional values in executable tool and MCP server definitions; runtime parsing remains tolerant of historical stored fields so upgrades do not break existing Sessions or replay. Custom-tool `input_schema` remains an open JSON Schema. Skills and multi-agent orchestration are not yet executed. |
