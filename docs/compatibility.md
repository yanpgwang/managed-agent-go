...
| Environment lifecycle | Supported | Create, get, list, archive, and delete support `cloud` and `self_hosted`; description, metadata, self-hosted scope, default response shapes, and delete responses round-trip through the official SDK. List supports archive filtering and forward cursor pagination. Cloud sandbox tools execute on the worker; self-hosted sandbox tools park for `user.tool_result` from the client. Configured networking/packages and Environment update remain incomplete. |
