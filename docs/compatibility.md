...
| Session Threads | Limited | All five official Get/List/Archive and Thread Event list/stream operations interoperate with the official Go SDK. Every Session receives a durable primary Thread with an independent Agent, status, usage, and timing projection in the same creation transaction. Every event is owned by one Thread while retaining a Session-wide total-order sequence; Session history and the current workflow are restricted to the primary ledger, and child event lists cannot leak primary activity. Atomic child creation captures a full Agent from the immutable Session roster and writes `session.thread_created` to the parent ledger. Archiving the runtime-produced primary Thread archives the idle Session. Child execution and live streaming, delegation, cross-posting, and targeted interrupts are not yet implemented. |
