...
| Agent configuration | Limited | Model, system, tools, MCP references, skills, metadata, and opaque `multiagent` values are stored. MCP references resolve to Session-pinned remote tool discovery; skills execution and multi-agent orchestration are not supported. |
...
| Session lifecycle | Supported | Create from latest or pinned agent versions, preserve an immutable resolved snapshot, get, list, update title, archive, and delete. Delete waits for durable sandbox teardown; workers autonomously resume a fenced deletion after an API-process crash. A provider outage can return an error while the cleanup Workflow continues and later reconciliation safely joins it. Several upstream response fields remain empty placeholders. |
...
| Built-in tool loop | Limited | `always_allow` `bash`, `read`, `write`, `edit`, `glob`, and `grep` execute as durable Temporal Activities. `web_fetch` and `web_search` use the configured Messages API endpoint as native server tools, retain provider-private continuation blocks, and currently require `always_allow` plus endpoint support. |
