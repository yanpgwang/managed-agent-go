...
)`, skill.DisplayTitle, skill.TitleExplicit).Scan(&titleConflict); err != nil {
			return err
		}
		if titleConflict {
...
INSERT INTO skills (
    ...
    display_title_explicit, ready
			...
			skill.Source, skill.TitleExplicit,
		); err != nil {
			return err
		}
...
WHERE skill_id = $1 AND version = $2 AND state <> 'ready'`, skillID, version)
	return err
}
...
WHERE skill.id = $1 AND NOT skill.ready
  AND NOT EXISTS (SELECT 1 FROM skill_versions WHERE skill_id = skill.id)`, id)
	return err
}
...
func (r *SkillRepository) DeleteSkill(ctx context.Context, id string) (domain.Skill, error) {
	skill, err := scanSkill(r.store.pool.QueryRow(ctx, `
DELETE FROM skills AS skill
WHERE skill.id = $1 AND skill.ready
...
RETURNING id, created_at, updated_at, display_title, latest_version, source,
          display_title_explicit, ready`, id))
	...
	var exists bool
	if queryErr := r.store.pool.QueryRow(ctx, `SELECT EXISTS(SELECT 1 FROM skills WHERE id = $1 AND ready)`, id).Scan(&exists); queryErr != nil {
